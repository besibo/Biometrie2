...
list.of.packages <- c("nycflights13", "gapminder", "knitr", "tidyverse", "gridExtra", "remotes", "extrafont", "kableExtra", "bindrcpp", "skimr")
...
```{r points, fig.cap="Retards à l'arrivée en fonction des retards au décollage pour les vols d'Alaska Airlines au départ de New York City en 2013.", tidy=FALSE, fig.pos = "!htpb"}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay)) + 
  geom_point()
```
...
```{r transparent, tidy=FALSE, warning = FALSE, fig.cap="La même figure, avec des points semi-transparents."}
...
```{r transpfactor, tidy=FALSE, warning=FALSE, fig.cap="Retards à l'arrivée pour les 12 mois de l'année 2013."}
ggplot(data = alaska_flights, 
...
```{r, exodiamonds, tidy=FALSE, warning=FALSE, echo = FALSE, fig.cap="Prix de 5000 diamants en fonction de leur taille en carats et de leur clarté."}
...
```{r wind, tidy=FALSE, fig.cap="Températures horaires et vitesse du vent à l'aéroport de Newark entre le 1er et le 15 janvier 2013. La couleur de la ligne et des points renseigne sur la vitesse du vent."}
ggplot(small_weather, aes(x = time_hour, y = temp, color = wind_speed)) +
  geom_line() +
  geom_point()
```

```{r wind2, tidy=FALSE, fig.cap="Températures horaires et vitesse du vent à l'aéroport de Newark entre le 1er et le 15 janvier 2013. La couleur de la ligne renseigne sur la vitesse du vent."}
ggplot(small_weather, aes(x = time_hour, y = temp)) +
  geom_line(aes(color = wind_speed)) +
  geom_point()
```

```{r wind3, tidy=FALSE, fig.cap="Températures horaires et vitesse du vent à l'aéroport de Newark entre le 1er et le 15 janvier 2013. La couleur des points renseigne sur la vitesse du vent."}
ggplot(small_weather, aes(x = time_hour, y = temp)) +
  geom_line() +
...
```{r rughist, warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Ajout des données brutes sous forme de 'tapis' (`rug`) sous l'histogramme."}
ggplot(weather, aes(x = temp)) +
  geom_histogram(bins = 60, color = "white") +
  geom_rug(alpha = 0.1)
```
...
```{r irregclasses, warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Spécification manuelle des limites de classes de tailles (classes irrégulières)."}
ggplot(weather, aes(x = temp)) +
...
```{r break, warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Un exemple d'utilisation de l'argument `breaks`."}
...
```{r wrap, warning=FALSE, tidy=FALSE, message=FALSE, fig.asp=1.2, fig.cap='(ref:wrap)'}
ggplot(weather, aes(x = temp, fill = factor(month))) +
  geom_histogram(bins = 20, color = "grey30") +
...
```{r grid, warning=FALSE, tidy=FALSE, message=FALSE, fig.asp=1.2, fig.cap="(ref:grid)"}
ggplot(weather, aes(x = temp, fill = factor(month))) +
  geom_histogram(bins = 20, color = "grey30") +
...
```{r tidy=FALSE, fig.cap="Un boxplot fort peu utile..."}
...
Le package `RColorBrewer` propose une large gamme de palettes de couleurs (figure \@ref(fig:brew)) :

```{r, brew, echo = FALSE, out.width='50%', fig.align='center', fig.cap="Toutes les palettes de couleur du package `RColorBrewer`."}
...
- [cases](https://besibo.github.io/Biometrie2/data/cases.rdata)
- [population](https://besibo.github.io/Biometrie2/data/population.rdata)
- [rates](https://besibo.github.io/Biometrie2/data/rates.rdata)
- [storms](https://besibo.github.io/Biometrie2/data/storms.rdata)
