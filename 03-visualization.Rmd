...
```{r gapmind, echo=FALSE, fig.cap="Espérance de vie en fonction du PIB par habitant en 2007"}
gapminder_2007 %>% 
...
```{r warning=FALSE, tidy=FALSE, fig.cap="Histogramme des températures enregistrées en 2013 dans les 3 aéroports de New York"}
...
```{r warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Utilisation des arguments `fill` et `color` pour modifier l'aspect de l'histogramme."}
ggplot(weather, aes(x = temp)) +
...
```{r warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Modification du nombre de classes"}
ggplot(weather, aes(x = temp)) +
...
```{r warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Ajout des données brutes sous forme de 'tapis' (`rug`) sous l'histogramme."}
ggplot(weather, aes(x = temp)) +
  geom_histogram(bins = 60, color = "white") +
  geom_rug(alpha = 0.1)
```
...
```{r warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Modification de la largeur des classes avec `binwidth`"}
ggplot(weather, aes(x = temp)) +
...
```{r warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Spécification manuelle des limites de classes de tailles (classes irrégulières)"}
ggplot(weather, aes(x = temp)) +
...
```{r warning=FALSE, tidy=FALSE, message=FALSE, fig.cap="Distribution des températures avec visualisation des données mensuelles."}
...
```{r barplot, tidy=FALSE, warning = FALSE, fig.cap="Barplot pour des données non pré-comptées."}
...
```{r barplotcol, tidy=FALSE, warning = FALSE, fig.cap="Barplot pour des données pré-comptées."}
...
Revenons à `nycflights13`. Imaginons que nous souhaitions connaître le nombre de vols affrétés par chaque compagnie aérienne au départ de New York en 2013. Dans le jeu de données `flights`, la variable `carrier` nous indique à quelle compagnie aérienne appartiennent chacun des 336776 vols ayant quitté New York en 2013. La façon la plus simple de représenter ces données est donc la suivante :

```{r bpcarrier, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ de New York en 2013."}
ggplot(data = flights, mapping = aes(x = carrier)) +
  geom_bar()
```

Il est généralement plus utile de trier les catégories par ordre décroissant. Nous pouvons faire cela facilement grâce à la fonction `fct_infreq` du package `forcats`. Si vous avez installé le `tidyverse`, le package `forcast` doit être disponible sur votre ordinateur. N'oubliez pas de la charger si besoin :

```{r bpcarriersorted, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ de New York en 2013."}
library(forcats)
...
Si nous souhaitons connaître le nombre de vols précis de chaque compagnie aérienne, il nous faut faire appel à plusieurs fonctions du package `dplyr` que nous détaillerons dans les chapitres suivants. Ci-dessous, nous créons un nouveau tableau contenant le nombre de vols de chaque compagnie aérienne et les compagnies sont ordonnées par nombre de vols décroissants :

```{r tidy = FALSE}
carrier_table <- flights %>%   # Pour créer une nouvelle table, on prend flights, puis...
  group_by(carrier) %>%        # On groupe les données par compagnie aérienne, puis...
  summarize(nombre = n()) %>%  # On calcule le nombre de vols par compagnie, puis ...
  arrange(desc(nombre))        # On trie le tableau par nombre de vols décroissant.
carrier_table                  # Enfin, on affiche la nouvelle table
```

Ici, la table a été triée par nombre de vols décroissants. Mais attention, **les niveaux** du facteur `carrier` n'ont pas été modifiés :

```{r}
factor(carrier_table$carrier)
```

Le premier niveau est toujours `9A`, puis `AA`, puis `AS`, et non l'ordre du tableau nouvellement créé (`UA`, puis `B6`, puis `EV`...) car les niveaux sont toujours triés par ordre alphabétique. La conséquence est que faire un barplot avec ces données et la fonction `geom_col()` ne permet pas d'ordonner les catégories correctement :

```{r bpcarriercol, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ de New York en 2013."}
...
```{r bpcarriersortedcol, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ de New York en 2013."}
ggplot(carrier_table, aes(x = fct_reorder(carrier, nombre, .desc = TRUE), y = nombre)) +
  geom_col()
```

Vous voyez donc que selon le type de données dont vous disposez (soit un tableau comme `flights`, avec toutes les observations, soit un tableau beaucou plus compact comme `carrier_table`), la démarche permettant de produire un diagramme bâton, dans lequel les catégories seront triées, sera différente.
...
À titre d'exemple, examinez ce diagramme, qui reprend les même chiffres que précédemment, et tentez de répondre aux questions suivantes :

```{r piechart, echo=FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ de New York en 2013"}
flights %>% 
  count(carrier) %>% 
...
Supposons par exemple que nous devions visualiser le nombre de vol de chaque compagnie aérienne, au départ de chacun des 3 aéroports de New York : John F. Kennedy (`JFK`), Newark (`EWR`) et La Guardia (LGA).

#### Graphique empilé

La méthode a plus simple est celle du graphique empilé :

```{r stacked, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
...
À mon sens, le graphique peut gagner en lisibilité si on ajoute une couleur pour le contour des barres :

```{r stacked2, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
...
```{r dodge, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
...
La meilleure alternative est probablement l'utilisation de `facet`s que nous avons déjà décrite à le section \@ref(facets) :

```{r barfacet, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1)
```

Ici, chaque graphique permet de comparer les compagnies aériennes au sein de l'un des aéroports de New York, et puisque l'ordre des compagnies aériennes est le même sur l'axe des `x` des 3 graphiques, une lecture verticlae permet de comparer aisément le nombre de vls qu'une compagnie donnée a affrété dans chacun ds 3 aéroports de New York.
...
- un titre pour chaque élément des légendes de couleurs, de forme, de taille, etc

Reprenons par exemple le graphique de la figure \@ref(fig:varcolor) :

```{r, varcolorlabel, tidy=FALSE, warning=FALSE, fig.cap="Association de `color` à une variable catégorielle"}
ggplot(alaska_flights, aes(x = dep_delay, y = arr_delay, color = factor(month))) +
  geom_point()
```

Nous pouvons ajouter sur ce graphique les éléments précisés plus haut en ajouter la fonction `labs()` sur une nouvelle couche du graphique :


```{r, varcolorlabel2, tidy=FALSE, warning=FALSE, fig.cap="Exemple d'utilisation de `labs()`"}
ggplot(alaska_flights, aes(x = dep_delay, y = arr_delay, color = factor(month))) +
  geom_point() +
  labs(title = "Relation linéaire positive entre le retard des vols au départ et à l'arrivée",
       subtitle = "L'essentiel des points est centré sur 0, mais certains retard dépassent 3 heures",
...
```{r barfacetbis, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
...
```{r barfacetgray, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
Le package `RColorBrewer` propose une large gamme de palettes de couleurs :

```{r, RcolorbrewerPalettes, echo = FALSE, out.width='50%', fig.align='center', fig.cap="Toutes les palettes de couleur du package `RColorBrewer`"}
knitr::include_graphics('images/brewer.png')
```

`ggplot2` permet d'appliquer ces palettes très simplement :

```{r barfacetbrewer, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
De même, le package `viridis` propose une palette de couleur intéressante qui maximise le contraste et facilite la disrimination des catégories pour les daltoniens. Là encore, `ggplot2` nous donne accès à cette palette :

```{r barfacetviridis, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
Enfin, si les palettes de couleur ne convienent pas, il est toujours possible de spécifier manuellement les couleurs souhaitées. R propose un accès rapide à 657 noms de couleurs. Pour afficher leurs noms, il suffit de taper :
...
```{r barfacetmanual, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
Outre ces 657 couleurs qui disposent d'un nom spécifique, il est possible de spécifier les couleurs en utilisant des codes hexadécimaux et des codes rgb (red, green, blue). De nombreux sites permettent de choisir n'importe quelle couleur dans une palette qui en compte des millions et d'obtenir de tels code. [Ce site](https://www.color-hex.com) permet de le faire très simplement :

```{r barfacethex, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Nombre de vols par compagnie aérienne au départ des 3 aéroports de New York en 2013."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
```{r, varcolorviridis, tidy=FALSE, warning=FALSE, fig.cap="Association de `color` à une variable catégorielle"}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay, color = factor(month))) +
  geom_point() +
  scale_color_viridis_d()
```

```{r, varcolorviridis2, tidy=FALSE, warning=FALSE, fig.cap="Association de `color` à une variable numérique"}
ggplot(data = alaska_flights, mapping = aes(x = dep_delay, y = arr_delay, color = arr_time)) +
  geom_point() +
...
Enfin, pour la figure \@ref(fig:varcolorviridis), c'est une variable catégorielle qui est associée à l'esthétique de couleur (`factor(month)`). La fonction utilisée pour modifier les couleurs doit donc en tenir compte : le `_d` à la fin de `scale_color_viridis_d()` signifie "discrete", c'est-à-dire "discontinue". À l'inverse, pour le graphique \@ref(fig:varcolorviridis2), c'est une variable numérique continue qui est associée à l'esthétique de couleur (`arr_time`). La fonction utilisée pour modifier les couleurs en est le reflet : le `_c` à la fin de `scale_color_viridis_c()` est l'abbréviation de "continuous", c'est-à-dire "continue".

Si vous ne voulez pas avoir de message d'erreur, attention donc, à choisir la fonction `scale_XXX_XXX()` appropriée. Pour cela, aidez-vous de l'aide que RStudio vous apporte en tapant les premières lettres de la fonction et en parcourant la liste des fonctions proposées dans le menu déroullant qui apparait sous votre curseur.


### Les thèmes

L'apparence de tout ce qui ne concerne pas directement les données d'un graphique est sous le contrôle d'un thème. Les thèmes contrôle l'apparence générale du graphique : quelles polices et tailles de caractères sont utilisées, quel arrière plan est utilisé poru le graphique, faut-il intégrer un quadrillage sous le graphique, et si oui, quelles doivent être ses caractéristiques ?

Il est posible de spécifier chaque élément manuellement. Nous nous contenterons ici de passer en revue quelques thèmes prédéfinis qui devraient couvrir la plupart de vos besoins. 

Reprenons par exemple le code de la figure \@ref(fig:barfacetbrewer) et ajoutons un titre :

```{r theme1, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Utilisation du thème par défaut : `theme_gray()`."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
       y = "Nombre de vols",
       fill = "Aéroports de\nNew York",
       title = "Couverture inégale des aéroports de New York par les compagnies aériennes") +
  scale_fill_brewer(palette = "Accent")
```
...
```{r themebw, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Utilisation du thème `theme_bw()`."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
- `theme_calssic()` : thème classique, avec des axes mais pas de quadrillage
...
```{r thememinimal, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Utilisation du thème minimaliste."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
L'argument `base_family` de chaque thème permet de spécifier une police de caractère différente de celle utilisée par défaut. Évidemment, vous ne pourrez utiliser que des polices qui sont disponibles sur l'ordinateur que vous utilisez. Dans l'exemple de la figure \@ref(fig:themefont) ci-dessous, j'utilise la police "Futura LT Book". Ci cette police n'est pas disponible sur votre ordinateur, ce code produira une erreur. Si c'est le cas, remplacez-la par une police de votre ordinateur. Attention, son nom exact doit être utilisé. Cela signifie bien sûr le respect des espaces, majuscules, etc.

```{r themefont, fig.asp = 1, tidy=FALSE, warning = FALSE, fig.cap="Modification de la police de caractère."}
ggplot(flights, aes(x = fct_infreq(carrier), fill = origin)) +
  geom_bar(color = "black") +
  facet_wrap(~origin, ncol = 1) +
  labs(x = "Compagnie aérienne",
...
Avec tout ça, vous devriez êtes armés pour produire 95% des graphiques dont vous aurez besoin tout au long de votre cursus universitaire. Un point important a pour l'instant été omis : l'ajout de barres d'erreurs sur vos graphiques. Nous verrons comment faire cela un peu plus tard, après avoir appris à manipuler efficacement des tableaux de données avec les packages `tidyr` et `dplyr`.
...
```{r exercice, echo=FALSE, tidy=FALSE, warning = FALSE}
small_flights %>% 
