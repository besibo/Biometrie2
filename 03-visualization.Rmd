...
3. À quoi correspondent les chiffres25, 50, 75, 100 ?
...
Dans un boxplot, les éléments sont les suivants :
...
Dernière chose concernant les boxplots : c'est une représentation graphique très proche de l'histogramme. Pour vous en convaincre, je représente ci-dessous uniquement les temperatures du mois de novembre :

```{r tidy=FALSE, warning = FALSE, message = FALSE, echo=FALSE, fig.cap="Distribution des températures de Novembre 2013"}
library(gridExtra)
...
À gauche un histogramme pour les températures de novembre (j'ai permuté les axes pour `y` porte la température pour les 3 graphiques), au centre, un boxplot pour ces mêmes données, et à droite, les données brutes. On voit bien que ces 3 représentations graphiques sont similaires. Toutes rendent compte du fait que les températures de Novembre sont majoritairement comprises entre 35 et 52 degrés farenheit. Au delà de cette fourchette (au-dessus comme en-dessous) les observations sont plus rares.

Le nuage de points affiche toutes les données. C'est donc lui le plus complet mais pas forcément le plus lisible. Les points sont en effet très nombreux et la lecture du graphique peut en être compliquée. L'histogramme simplifie ces données en les regroupant dans des classes. On constate toujours 2 pics qui correspondent aux zones plus denses du nuage de points. Le boxplot enfin synthétise encore plus ces données. Elles sont résumées par 7 valeurs : le minimum, le maximum, les 3 quartiles, et les bornes de l'intervalle de confiance à 95% de la médiane. C'est une représentation très synthétique qui nous permet de comparer beaucoup de catégories côte à côte (voir la figure \@ref(fig:notchedboxplot) un peu plus haut), mais qui est forcément moins précise qu'un histogramme. Vous noterez toutefois que la boîte du boxplot recouvre en grande partie la zone des 2 pics de l'histogramme. En outre, sur la figure \@ref(fig:notchedboxplot), la tendance générale est très visible : il fait plus chaud en été qu'en hiver (étonnant non ?).
