...
Seules les lignes contenant `NA` dans la colonne `dep_time` sont retenues. Il y a donc `r nycflights13::flights %>% filter(is.na(dep_time)) %>% nrow()` vols qui n'ont finalement pas décollé. 
...
Il y a donc `r nycflights13::flights %>% filter(!is.na(dep_time) & is.na(arr_time)) %>% nrow()` vols qui ne sont pas arrivés à destination (soit moins de 0,2% des vols au départ de New York en 2013). Selon vous, quelles raisons peuvent expliquer qu'un vol qui a décollé n'ait pas d'heure d'atterrissage ?
...
La température moyenne est donc de `r round(mean(nycflights13::weather$temp, na.rm = TRUE),1)` degrés Farenheit et l'écart-type vaut `r round(sd(nycflights13::weather$temp, na.rm = TRUE),1)` degrés Farenheit.
...
Y a-t'il un lien entre le gain de temps et la vitesse en vol ou la distance ? Pour le savoir, il suffit de faire quelques graphiques.
